...
class TickerStreamRepositoryImpl @Inject constructor(
    private val webSocketClient: WebSocketClient,
) : TickerStreamRepository {
    override val connectionState: Flow<TickerConnectionState> = webSocketClient.connectionState.map { state ->
        when (state) {
            ...
            is ConnectionState.Error -> TickerConnectionState.Error(
                errorMsg = state.errorMsg
            )

            is ConnectionState.Reconnecting -> TickerConnectionState.Reconnecting(
    ...
    override val event: Flow<TickerStreamEvent> = webSocketClient.messages.map { message ->
        when (message) {
    ...
    override fun subscribe(ticker: String): Boolean {
        return webSocketClient.subscribe(ticker)
    }

    override fun unsubscribe(ticker: String): Boolean {
        return webSocketClient.unsubscribe(ticker)
    }
}
