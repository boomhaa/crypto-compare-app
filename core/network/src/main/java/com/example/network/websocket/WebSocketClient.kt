...
class WebSocketClient @Inject constructor(
    ...
    private val _messages = MutableSharedFlow<SocketDtoMessage>(
    ...
    fun unsubscribe(ticker: String): Boolean {
        ...
        val wasRemoved = synchronized(lock) {
    ...
    private fun sendMessage(type: MessageType, ticker: String): Boolean {
        val payload = SocketOutboundMessage(
            type = type.type,
    ...
    private fun parseAndEmitRaw(rawMessage: String) {
        ...
        val message: SocketDtoMessage =
            when (type) {
                MessageType.WELCOME -> {
                    val data = runCatching { gson.fromJson(parsedMessage.data, WelcomeData::class.java) }.getOrNull()
                        ?: WelcomeData("")
                    SocketDtoMessage.Welcome(parsedMessage.id, data = data)
                }

                MessageType.SUBSCRIBE -> {
                    val data = runCatching { gson.fromJson(parsedMessage.data, TickerData::class.java) }.getOrNull()
                        ?: TickerData("")
                ...
                MessageType.PRICE_CHANGE -> {
                    val data =
                        runCatching { gson.fromJson(parsedMessage.data, SymbolPriceChangeData::class.java) }.getOrNull()
                            ?: return
                    SocketDtoMessage.SymbolPriceChange(parsedMessage.id, data = data)
                }

                MessageType.ERROR -> {
                    val data = runCatching { gson.fromJson(parsedMessage.data, ErrorData::class.java) }.getOrNull()
